...
kwargs = dict(sigma=sigma, gamma=gamma, R0=R0, Rt_smooth=sm, smooth_days=ds, ret_Rt=True, population=population, smooth=True)
...
st.line_chart(pd.DataFrame({'cases': np.maximum(cases.diff(), 0), 'smooth': smoothed_diff(cases)}))
st.line_chart(Rt)
st.line_chart(data)
